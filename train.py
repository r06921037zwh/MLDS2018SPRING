...
def read_testing_json():
    ...
    indir = 'D:/Downloads/MLDS_hw2_1_data/testing_data/feat'
    for root, dirs, filenames in os.walk(indir):
            for f in filenames:
                test_features[count,:] = np.load(os.path.join(indir,f))
                count+=1
                for i in json_testing_data:                    
                    if i['id'] == f.replace(".npy",""):
                        test_labels.append(i["caption"])   
...
#test_features, test_labels = read_testing_json()
...
class Video_Caption_Generator():
    ...
    def build_generator(self):
        ...
        for i in range(self.n_caption_lstm_step):
            ...
            output2 += output3
            """
            temp = self.embed_word_W
            temp[:,1].assign(tf.zeros([lstm_size,1]))
            logit_words = tf.nn.xw_plus_b(output2,temp, self.embed_word_b)
...
def train():
    ...
    saver = tf.train.Saver(max_to_keep=30, write_version=1)
    saver.restore(sess, './my_models/model_100nn/model-420')
    ...
    for epoch in range(421, 421+n_epochs):
        loss_to_draw_epoch = []
...
def test(model_path='./my_models/model_100nn/model-420'):
    tf.reset_default_graph()
