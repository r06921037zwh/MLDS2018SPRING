    ...
    def train(self, n_epochs):  
        ...
        with tf.Session() as sess:
            ...
            for i in range(20):
                ...
                tr_noise = np.random.normal(0 , 0.1 , [55000 , 784])
                loss , acc = sess.run([self.loss, self.accuracy] ,feed_dict={self.img: mnist.train.images + tr_noise , self.label: mnist.train.labels})                
                Noise_loss.append(loss)
            Noise_loss = np.array(Noise_loss)
            train_loss , train_acc = sess.run([self.loss, self.accuracy] ,feed_dict={self.img: mnist.train.images , self.label: mnist.train.labels})
            test_loss , test_acc = sess.run([self.loss, self.accuracy] ,feed_dict={self.img: mnist.test.images , self.label: mnist.test.labels})
            loss_max = np.max(Noise_loss)
            sharpness = (loss_max - train_loss) / (1 + train_loss)
            print(sharpness)
            print(Noise_loss)
            #writer.close()
            return train_loss , train_acc , test_loss , test_acc , sharpness
...
if __name__ == '__main__':
    ...
    model = ConvNet(batch_size=50, store_path='model', store_name='m1')
    model.build()
    ...
    model.build()
    tr_loss , tr_acc , te_loss , te_acc , sharpness = model.train(n_epochs=10)
    ...
    model.build()
    tr_loss , tr_acc , te_loss , te_acc , sharpness = model.train(n_epochs=20)
    ...
    model.build()
    tr_loss , tr_acc , te_loss , te_acc , sharpness = model.train(n_epochs=30)
    ...
    ax1.plot(batch , train_loss , "b" , label = 'train')
    ax1.plot(batch , test_loss , "b--" ,  label = 'test' )
    ax2.plot(batch , sharp , "r" , label = 'sharpness')
...
################################################################
    ...
    ax3.plot(batch , train_acc , "b" , label = 'train')
    ax3.plot(batch , test_acc , "b--" ,  label = 'test' )
    ax4.plot(batch , sharp , "r" , label = 'sharpness')
